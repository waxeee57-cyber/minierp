...
class SearchController extends Controller
{
    public function __invoke(Request $request): JsonResponse
    {
        ...
        $customers = Customer::query()
            ->where(fn ($q) => $q->where('company', 'like', $like)->orWhere('name', 'like', $like)->orWhere('email', 'like', $like)->orWhere('city', 'like', $like))
        ...
        $orders = Order::query()->with('customer:id,name,company')
            ...
            ->orWhereHas('customer', fn ($q) => $q->where('company', 'like', $like))
        ...
        $invoices = Invoice::query()->where('number', 'like', $like)->latest('id')->limit(5)->get()
